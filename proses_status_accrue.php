...
$query_hapus = "DELETE FROM tbl_nopin_accrue";
...
if(isset($_FILES['berkas_status']['name']) && in_array($_FILES['berkas_status']['type'], $file_mimes)) {
	...
	for($i = 1;$i < count($sheetData);$i++)
	{
        ...
        mysqli_query($koneksi, "UPDATE tbl_accrue SET account_sts='$account_sts' WHERE no_pin=$no_pin");

        // masukkan data ke tbl_nopin
        $q_simpan = "INSERT INTO tbl_nopin_accrue(no_pin, account_status, app_date) VALUES('$no_pin', '$account_sts', '$app_date')";
        mysqli_query($koneksi, $q_simpan) or die ((mysqli_error($koneksi)));
    }
    
    echo '<script>
        alert("Account Status Berhasil Diupdate");window.location="data_feeding_accrue.php";
    </script>';
