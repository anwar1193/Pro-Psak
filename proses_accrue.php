...
if(isset($_FILES['berkas_excel']['name']) && in_array($_FILES['berkas_excel']['type'], $file_mimes)) {
	...
	for($i = 1;$i < count($sheetData);$i++)
	{
        ...
        $accrue_restru = $sheetData[$i]['12'];
        $tanggal_upload = date('Y-m-d');
        $status_generate = 'belum';

        // Cek Apakah Ada Data Yang Sama
        $query_cek = "SELECT * FROM tbl_accrue WHERE no_pin='$no_pin'";
        ...
        if($cek > 0){ // jika ada nopin yang sudah ada sebelumnya
            $query_batal = "DELETE FROM tbl_accrue WHERE status_generate = 'belum'";
            ...
            mysqli_query($koneksi, "insert into tbl_accrue(no_pin, no_rek, account_sts, kode_cabang, cabang, account_name, restru_date, booking_date, sisa_tenor, tgl_jatuh_tempo, fincat, accrue_restru, tanggal_upload, status_generate) 
            values ('$no_pin','$no_rek','$account_sts', '$kode_cabang', '$cabang', '$account_name', '$restru_date', '$booking_date', $sisa_tenor, '$tgl_jatuh_tempo', '$fincat', $accrue_restru, '$tanggal_upload', '$status_generate')");
        }
    ...
    echo '<script>
        alert("Data Berhasil Diupload");window.location="generate_accrue.php";
    </script>';
