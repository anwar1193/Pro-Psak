...
$query_hapus = "DELETE FROM tbl_nopin_jf";
...
if(isset($_FILES['berkas_status']['name']) && in_array($_FILES['berkas_status']['type'], $file_mimes)) {
	...
	for($i = 1;$i < count($sheetData);$i++)
	{
        ...
        mysqli_query($koneksi, "UPDATE tbl_jf SET account_sts='$account_sts' WHERE no_pin=$no_pin");

        // masukkan data ke tbl_nopin
        $q_simpan = "INSERT INTO tbl_nopin_jf(no_pin, account_status, app_date, bank_awal) VALUES('$no_pin', '$account_sts', '$app_date', '$account_sts')";
        mysqli_query($koneksi, $q_simpan) or die ((mysqli_error($koneksi)));
    }
    
    echo '<script>
        alert("Account Status JF Berhasil Diupdate");window.location="data_feeding_jf.php";
    </script>';
