...
<?php
	...
	$query = "SELECT NoPin,JFORPLEDGEBANK FROM [DLEAS].[dbo].[v_rptAllDataCreditReview] WHERE NoPin IN ('".substr($noPin,0,strlen($noPin)-3)."')";
	...
	while($row = sqlsrv_fetch_array($result)){
		...
		$nilai1 = $row['NoPin'];
		$nilai2 = $row['JFORPLEDGEBANK'];
		$nilai3 = $dateDleas;
		$nilai4 = $waktu_tarik_data;

		// pengiriman ke situsku.com via CURL
		$url = "10.20.0.30/Pro-Psak/transfer_dleas/transfer_nopin_jf/terima.php";
...
<?php 
	...
	echo "<script>
		...
		document.location.href = '../../data_accStatusJF.php';
	</script>";
