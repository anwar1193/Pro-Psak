<?php  
	...
	header("Content-Disposition: attachment; filename=prediksi_accrue.xls");
...
<table border = '1' width="100%">
    ...
    <?php  
        $no = 0;
        $sql = "SELECT DISTINCT bulan,tahun FROM tbl_accrue_detail WHERE status_paid='belum'";
        ...
        while($data = mysqli_fetch_array($res)){
            ...
            $query_sum = "SELECT 
                            ...
                            FROM tbl_accrue_detail WHERE bulan='$bulan' AND tahun='$tahun' AND status_paid='belum'";
            
            $res_sum = mysqli_query($koneksi, $query_sum) or die('error sum');
            $data_sum = mysqli_fetch_array($res_sum);

            // Grand Total
            $gt_accrue_restru += $data_sum['t_accrue_restru'];
    ...
    <tr>
        ...
        <td style="text-align:right"><?php echo number_format($data_sum['t_accrue_restru'],0,'.',',') ?></td>
    </tr>
    <?php } ?>
    ...
    <tr style="background-color: #b6fc03">
        ...
        <th style="text-align:right"><?php echo number_format($gt_accrue_restru,0,'.',',') ?></th>
    </tr>
